    ...
    public void Update()
    {
        if (state > state_.NO_JOYCONS)
        {
            ...
            while (reports.Count > 0)
            {
                ...
                ts_de = report_buf[1];
                DebugPrint(string.Format("Dequeue. Queue length: {0:d}. Packet ID: {1:X2}. Timestamp: {2:X2}. Lag to dequeue: " + System.DateTime.Now.Subtract(rep.GetTime()).ToString() + ". Lag between packets (expect 15ms): " + rep.GetTime().Subtract(ts_prev).ToString(),
                    reports.Count, report_buf[0], report_buf[1]), DebugType.THREADING);
                ts_prev = rep.GetTime();
            }
